...
function query (str, since, etag, force, code, count, originalURL) {
  const uri = trim(str)
  if (!uri) return null
...
function Query (url, since, etag, force = false, code, count = 0, originalURL) {
  if (!(this instanceof Query)) {
    return new Query(url, since, etag, force, code, count, originalURL)
  }
  this.url = trim(url)
...
Query.prototype.redirect = function (code, url) {
...
Queries.prototype._transform = function (chunk, enc, cb) {
  ...
  while (index < buf.length) {
    ...
    if (this._start > -1 && end > -1) {
      ...
      index = 0
      if (term) {
        const qry = query(term.url, term.since)
        if (qry) {
          this.push(qry)
        } else {
