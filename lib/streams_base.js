...
function processQuery (ctx, qry) {
  if (!qry) throw new Error()

  if (!(qry instanceof Query)) {
    if (qry instanceof Buffer) qry = ctx.decoder.write(qry)
    if (typeof qry === 'string') {
      qry = new Query(qry)
    } else { // plain objects are fine too
      qry = new Query(qry.url, qry.since, qry.etag, qry.force)
    }
  }

  if (qry) {
    if (ctx.force) qry.force = true
    const r = ctx.redirects.get(qry.url)
  ...
  _transform (q, enc, cb) {
    ...
    getETag(this.db, uri, (er, etag) => {
      ...
      if (!qry.force && qry.etag) {
        ...
        // of longer periods of time spreading the load. We learn, streams cannot
