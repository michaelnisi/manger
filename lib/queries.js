  ...
  while (index < buf.length) {
    ...
    if (this._start > -1 && end > -1) {
      ...
      try {
        ...
        // TODO: not sure if we really should throw
...
// Normalize timestamp to Unix time
// - ts Unix time || IETF-compliant RFC 2822 timestamp
function time (ts) {
...
function tuple (term) {
  ...
  try {
    ...
    // TODO: not sure if we really should throw
    this.emit('error', new Error('Bad Date'))
    t = 0
  }
  return [term.url, t]
}
